    ...
    <div class="row align-items-center mb-3">
        <div class="col">
            @foreach ($gradeLevels as $gradeLevel)
                @if ($cur == url("/registrar/section/".Str::lower($gradeLevel->grade_name)))
                    <h1 class="h3 text-gray-800 py-2">{{ $gradeLevel->grade_name }}</h1>
                @endif
            @endforeach
        </div>
        <div class="col">
                ...
                <table class="table table-bordered table-hover" id="section-table">
                    ...
                    <tbody>
                        @foreach ($sections as $section)
                             @if ($cur == url("/registrar/section/".Str::lower($section->gradeLevel->grade_name)))
                             <tr>
